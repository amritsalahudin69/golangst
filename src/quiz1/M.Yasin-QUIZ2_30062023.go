...
type ArticleA struct {
	ArticleAID     uint            `json:"ArticleAId"`
	AuthorA        *AuthorA        `json:"AuthorA"`
	GameA          *GameA          `json:"GameA,omitempty"`
	GenreA         *GenreA         `json:"GenreA,omitempty"`
	PlatformA      *PlatformA      `json:"PlatformA,omitempty"`
	ImageArticleAA *ImageArticleAA `json:"ImageArticleAA"`
	TranslationR   *TranslationR   `json:"TranslationR"`
}
...
type GameA struct {
	...
	GenreA    *GenreA    `json:"GenreA"`
	PlatformA *PlatformA `json:"PlatformA"`
}
...
func (ArticleA *ArticleA) AddIdArticleA(id uint) {
	ArticleA.ArticleAID = id
}

func (ArticleA *ArticleA) AddAuthorAArticleA(AuthorA *AuthorA) {
	ArticleA.AuthorA = AuthorA
}

func (ArticleA *ArticleA) AddImageArticleAA(image *ImageArticleAA) {
	ArticleA.ImageArticleAA = image
}

func (ArticleA *ArticleA) AddTranslationRArticleA(TranslationR *TranslationR) {
	ArticleA.TranslationR = TranslationR
}

func (ArticleA *ArticleA) GetArticleA() {

	jsonData, err := json.MarshalIndent(ArticleA, "", "  ")
	if err != nil {
...
func main() {
	...
	ImageArticleAA := `{
		"ImageArticleAA": {
			"image": "https://api.duniaGameAs.co.id/api/content/upload/file/9874800061688113932.jpg",
			"imageDesktop": "https://api.duniaGameAs.co.id/api/content/upload/file/9074251731688113932.jpg",
			"imageHighlight": "https://api.duniaGameAs.co.id/api/content/upload/file/9842838701688113932.jpg"
		}
	}`
	fmt.Println(ImageArticleAA)
	AuthorA := `{
		"AuthorA":{
			"fullName":"haris13",
			"profilePicture":"https:\/\/api.duniaGameAs.co.id\/api\/user\/upload\/image\/223816331649748676.jpg",
	...
	TranslationR := `{
		"TranslationR":{
			...
			"MetaR":{
			   "title":"Benedetta Mobile Legends (ML) Hero: Skill, Build dan Kelebihan",
			   "keyword":"Benedetta",
			   "description":"Penasaran ingin tahu tentang hero Benedetta Mobile Legends (ML)? Lihat artikel Dunia GameAs dan pelajari tentang skills, build, dan kelebihannya.",
			   "canonicalURL":null
			}
		 }
	}`
